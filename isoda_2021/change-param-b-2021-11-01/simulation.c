...
double g_left(double t, double x_i0, double x_i, double x_ip1, double v, double theta, double param_a, double param_b, double l);
double g_inside(double t, double x_i0, double x_im1, double x_i, double x_ip1, double v, double theta, double param_a, double param_b, double l);
double g_right(double t, double x_i0, double x_im1, double x_i, double v, double theta, double param_a, double param_b, double l);
...
int main()
{
  ...
  for (Time = t_start; Time < t_max; Time += dt)
  {
    ...
    for (s = 0; s < N; s++) // l1を代入
    {
      if (s == 0) // 左のブロック
      {
        rk_l[s][0] = dt * g_left(Time, x_initial[s], x[s], x[s + 1], v[s], theta[s], param_a[s], param_b[s], l[s]);
      }
      else if (s == N - 1) // 右のブロック
      {
        rk_l[s][0] = dt * g_right(Time, x_initial[s], x[s - 1], x[s], v[s], theta[s], param_a[s], param_b[s], l[s]);
      }
      else // 左右以外のブロック
      {
        rk_l[s][0] = dt * g_inside(Time, x_initial[s], x[s - 1], x[s], x[s + 1], v[s], theta[s], param_a[s], param_b[s], l[s]);
      }
    }
    for (s = 0; s < N; s++) // m1を代入
    {
    ...
    for (s = 0; s < N; s++) // l2を代入
    {
      if (s == 0) // 左のブロック
      {
        rk_l[s][1] = dt * g_left(Time + 0.5 * dt, x_initial[s], x[s] + 0.5 * rk_k[s][0], x[s + 1] + 0.5 * rk_k[s + 1][0], v[s] + 0.5 * rk_l[s][0], theta[s] + 0.5 * rk_m[s][0], param_a[s], param_b[s], l[s]);
      }
      else if (s == N - 1) // 右のブロック
      {
        rk_l[s][1] = dt * g_right(Time + 0.5 * dt, x_initial[s], x[s - 1] + 0.5 * rk_k[s - 1][0], x[s] + 0.5 * rk_k[s][0], v[s] + 0.5 * rk_l[s][0], theta[s] + 0.5 * rk_m[s][0], param_a[s], param_b[s], l[s]);
      }
      else // 左右以外のブロック
      {
        rk_l[s][1] = dt * g_inside(Time + 0.5 * dt, x_initial[s], x[s - 1] + 0.5 * rk_k[s - 1][0], x[s] + 0.5 * rk_k[s][0], x[s + 1] + 0.5 * rk_k[s + 1][0], v[s] + 0.5 * rk_l[s][0], theta[s] + 0.5 * rk_m[s][0], param_a[s], param_b[s], l[s]);
      }
    }
    for (s = 0; s < N; s++) // m2を代入
    {
    ...
    for (s = 0; s < N; s++) // l3を代入
    {
      if (s == 0) // 左のブロック
      {
        rk_l[s][2] = dt * g_left(Time + 0.5 * dt, x_initial[s], x[s] + 0.5 * rk_k[s][1], x[s + 1] + 0.5 * rk_k[s + 1][1], v[s] + 0.5 * rk_l[s][1], theta[s] + 0.5 * rk_m[s][1], param_a[s], param_b[s], l[s]);
      }
      else if (s == N - 1) // 右のブロック
      {
        rk_l[s][2] = dt * g_right(Time + 0.5 * dt, x_initial[s], x[s - 1] + 0.5 * rk_k[s - 1][1], x[s] + 0.5 * rk_k[s][1], v[s] + 0.5 * rk_l[s][1], theta[s] + 0.5 * rk_m[s][1], param_a[s], param_b[s], l[s]);
      }
      else // 左右以外のブロック
      {
        rk_l[s][2] = dt * g_inside(Time + 0.5 * dt, x_initial[s], x[s - 1] + 0.5 * rk_k[s - 1][1], x[s] + 0.5 * rk_k[s][1], x[s + 1] + 0.5 * rk_k[s + 1][1], v[s] + 0.5 * rk_l[s][1], theta[s] + 0.5 * rk_m[s][1], param_a[s], param_b[s], l[s]);
      }
    }
    for (s = 0; s < N; s++) // m3を代入
    {
    ...
    for (s = 0; s < N; s++) // l4を代入
    {
      if (s == 0) // 左のブロック
      {
        rk_l[s][3] = dt * g_left(Time + dt, x_initial[s], x[s] + rk_k[s][2], x[s + 1] + rk_k[s + 1][2], v[s] + rk_l[s][2], theta[s] + rk_m[s][2], param_a[s], param_b[s], l[s]);
      }
      else if (s == N - 1) // 右のブロック
      {
        rk_l[s][3] = dt * g_right(Time + dt, x_initial[s], x[s - 1] + rk_k[s - 1][2], x[s] + rk_k[s][2], v[s] + rk_l[s][2], theta[s] + rk_m[s][2], param_a[s], param_b[s], l[s]);
      }
      else // 左右以外のブロック
      {
        rk_l[s][3] = dt * g_inside(Time + dt, x_initial[s], x[s - 1] + rk_k[s - 1][2], x[s] + rk_k[s][2], x[s + 1] + rk_k[s + 1][2], v[s] + rk_l[s][2], theta[s] + rk_m[s][2], param_a[s], param_b[s], l[s]);
      }
    }
    for (s = 0; s < N; s++) // m4を代入
    {
